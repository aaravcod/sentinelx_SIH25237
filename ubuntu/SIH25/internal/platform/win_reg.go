...
func (w *WindowsHardener) CheckRegistry(keyPath, valueName string, expectedValue interface{}) (bool, error) {
	...
	if valType == registry.DWORD || valType == registry.QWORD {
		...
		if val == uint64(expectedInt) { return true, nil }
		...
		if expectedStr, ok := expectedValue.(string); ok {
			if val == expectedStr { return true, nil }
		}
	}
...
func (w *WindowsHardener) RunCommand(cmdStr string, args []string, expectPattern string) (bool, error) {
...
func (w *WindowsHardener) CheckFilePermission(path, expectedMode string) (bool, error) {
	return true, nil 
}
